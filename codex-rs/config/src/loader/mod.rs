...
/// - tree      parent directories up to root looking for `./.empatra/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.empatra/config.toml` (loaded but disabled when untrusted)
