...
use crate::PROJECT_CONFIG_DIR_NAME;
...
const PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME: &str = PROJECT_CONFIG_DIR_NAME;
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME)
}
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_project_config*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_project_config*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_project_config*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_project_config = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_project_config)
                    .into_iter()
            ...
            // Example: if `<root>/.empatra -> <root>/decoy`, bwrap must still see
            // `<root>/.empatra`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.empatra` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_project_config: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .empatra read-only to the agent by
    ...
    let top_level_project_config = writable_root.join(PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME);
    if protect_missing_project_config || top_level_project_config.as_path().is_dir() {
        subpaths.push(top_level_project_config);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_project_config() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_project_config = expected_root.join(PROJECT_CONFIG_DIR_NAME);
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_project_config)
        );
    }
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_project_config_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_project_config = expected_root.join(PROJECT_CONFIG_DIR_NAME);
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_project_config.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&PROJECT_CONFIG_DIR_NAME.to_string()),
            "explicit .empatra rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_project_config),
            "explicit .empatra rule should win over the default protected carveout"
        );
        assert!(policy.can_write_path_with_cwd(
            explicit_project_config.join("config.toml").as_path(),
            cwd.path()
        ));
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let project_config = cwd.path().join(PROJECT_CONFIG_DIR_NAME).join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&project_config, cwd.path()));
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_project_config*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".empatra/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let project_config_dir = real_root.join(PROJECT_CONFIG_DIR_NAME);

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&project_config_dir).expect("create .empatra");
        ...
        let expected_project_config = link_root.join(PROJECT_CONFIG_DIR_NAME);
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-project-config");
        let project_config = root.join(PROJECT_CONFIG_DIR_NAME);
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &project_config).expect("create .empatra symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_project_config = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(PROJECT_CONFIG_DIR_NAME),
        )
        .expect("absolute .empatra symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let project_config_dir = real_tmpdir.join(PROJECT_CONFIG_DIR_NAME);
