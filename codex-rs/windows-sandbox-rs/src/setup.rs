...
mod tests {
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_project_config = extra_write_root.join(".empatra");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_project_config).expect("create extra .empatra");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_project_config).expect("canonical extra .empatra"),
                explicit_deny,
            ]
            .into_iter()
