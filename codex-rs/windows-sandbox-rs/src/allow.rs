...
mod tests {
    ...
    #[test]
    fn denies_project_config_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let project_config_dir = command_cwd.join(".empatra");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&project_config_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&project_config_dir).unwrap(),
