...
- `agentRole/list` — list user-defined agent roles. Each role includes its optional fixed `modelReasoningEffort` alongside `model`; `null` means the spawned agent inherits the caller's reasoning effort.
- `agentRole/write` — create or replace a user-defined agent role and persist its optional `modelReasoningEffort` as `model_reasoning_effort` in the role TOML file.
- `agentRole/delete` — delete a user-defined agent role.
...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, taskTitle}` describing an Agent V2 lifecycle event. `taskTitle` is the short human-readable title supplied when the agent is spawned and may be null for historical items.
- `workSwarmActivity` — `{id, runId, status, ..., tasks}` describing durable Work Swarm progress. Each task includes `taskTitle`, the short human-readable title supplied in the swarm DAG, and may contain null for historical items.
...
For linked Git worktrees, project hook declarations come from the matching `.empatra/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
