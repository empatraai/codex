...
impl PluginRequestProcessor {
    ...
    async fn plugin_list_response(
        ...
        let include_created_by_me_remote = plugins_input.openai_marketplaces_enabled
            && marketplace_kinds.contains(&PluginListMarketplaceKind::CreatedByMeRemote)
            && config.features.enabled(Feature::RemotePlugin);
        let include_global_remote = plugins_input.openai_marketplaces_enabled
            ...
            match tokio::task::spawn_blocking(move || {
                let outcome = plugins_manager_for_marketplace_listing
                    .list_marketplaces_for_config(
                        ...
                        plugins_input.openai_marketplaces_enabled && !use_remote_global_catalog,
                    )?;
                Ok::<
                    (
