...
impl PluginsManager {
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.openai_marketplaces_enabled
    ...
    pub fn maybe_start_plugin_startup_tasks_for_config(
        ...
        if config.plugins_enabled {
            let use_remote_global_catalog = config.openai_marketplaces_enabled
                && config.remote_plugin_enabled
                && auth_manager.current_auth_uses_codex_backend();
            if config.openai_marketplaces_enabled && !use_remote_global_catalog {
    ...
    fn marketplace_roots(
        ...
        let curated_marketplace_path =
            if config.openai_marketplaces_enabled && include_openai_curated {
                if matches!(
                    self.auth_mode(),
