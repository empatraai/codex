...
const DEFAULT_MULTI_AGENT_V2_TOOL_NAMESPACE: &str = "empatra_collaboration";
const DEFAULT_MULTI_AGENT_V2_SHARED_USAGE_HINT_TEXT: &str = r#"Note that collaboration tools cannot be called from inside `functions.exec`. Call `spawn_agent`, `send_message`, `followup_task`, `wait_agent`, `interrupt_agent`, `list_agents`, and the Work Swarm tools only as direct tool calls using the recipient shown in their tool definitions, such as `to=functions.empatra_collaboration.spawn_agent`, since they are intentionally absent from the `functions.exec` `tools.*` namespace. Available tools in `functions.exec` are explicitly described with a `tools` namespace in the developer message.
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .empatra/ folders.
