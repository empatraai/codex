...
//! - sensitive subpaths such as `.git`, `.agents`, and `.empatra` remain
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.empatra`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(PROJECT_CONFIG_DIR_NAME)
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let logical_codex = logical_home.join(".empatra");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".empatra"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::project_roots(Some(".empatra".into())),
                },
                access: FileSystemAccessMode::Read,
            },
        ]);
        ...
        let project_config = path_to_string(&temp_dir.path().join(".empatra"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&project_config));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&project_config)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.empatra"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.empatra"),
            ]
        );
        assert_eq!(
            args.args,
